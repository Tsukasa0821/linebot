...
def batch_add_work_tasks(content: str) -> str:
    ...
    result = []
    if created: result.append(f"✅ 已新增 {len(created)} 筆：\n" + "\n".join(f"  • {c}" for c in created))
    if errors: result.append("❌ 失敗：\n" + "\n".join(f"  • {e}" for e in errors))
    return "\n".join(result)
